...
#constructor
func Ability(global_positionn, rotationn, stackedd):
	position_offset = global_positionn
	...
	if get_tree().root.get_node("Main").my_ID == 1:
		...
		if (age >= 2.5 and not full):
			...
			for i in range(len(objects_touching)):
				if objects_touching[i] == get_parent():
					continue
		...
		if result:
			...
			if override_collision_position == Vector2(-500000, -500000):
				$CollisionShape2D.position = get_parent().to_local(Vector2(distance/2 * cos(get_parent().rotation) + start_point.x, distance/2 * sin(get_parent().rotation) + start_point.y))
			...
			$CollisionShape2D.position = get_parent().to_local(Vector2(max_len/2 * cos(get_parent().rotation) + start_point.x, max_len/2 * sin(get_parent().rotation) + start_point.y))
